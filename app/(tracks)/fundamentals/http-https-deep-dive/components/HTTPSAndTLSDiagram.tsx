    ...
    <div className="bg-[#1A1C1E] border border-divider rounded-[2px] p-6 lg:p-10 flex flex-col items-center">
      ...
      <div className="relative w-full max-w-3xl h-[400px] border border-divider bg-[#15171a] rounded-[2px] flex items-center justify-between px-8 md:px-20 mb-10 overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center gap-3">
          <div className="w-16 h-16 rounded-[2px] bg-[#282A2D] border border-divider-hover flex items-center justify-center shadow-lg">
            <svg
              className="w-6 h-6 text-platinum"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
          ...
          <span className="text-xs font-medium text-secondary uppercase tracking-widest font-mono text-center">
            Browser
            <br />
            <span className="text-[10px] text-muted normal-case font-sans opacity-70">
              Client
            </span>
          </span>
        </div>
      ...
      <div className="max-w-3xl w-full">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-lg font-heading text-platinum">
            ...
            return (
              <div key={step.id} className="flex flex-col gap-2">
                <div className="h-1 w-full rounded-full bg-surface/50 overflow-hidden">
                  <motion.div
                    className={cn(
                      "h-full w-full bg-accent origin-left",
                      currentStep === 4 ? "bg-emerald-400" : "",
                    )}
                    initial={{ scaleX: 0 }}
        ...
        <div className="mt-8 p-4 bg-surface/20 border border-divider/50 rounded-[2px] min-h-[100px] flex items-center">
          <p className="text-sm text-secondary leading-relaxed">
            {currentStep === 0 &&
              "Without HTTPS, your HTTP requests are sent in &apos;plaintext&apos;. Anyone on the network (like a cafe Wi-Fi or ISP) can read the passwords, headers, and data you send. Click &apos;Start Handshake&apos; to see how TLS secures it."}
            {currentStep === 1 && (
              <>
                <strong className="text-platinum">Step 1:</strong> The client
                reaches out over TCP port 443 and proposes TLS versions and
                cipher suites it supports to start encryption.
              </>
            )}
            {currentStep === 2 && (
              <>
                <strong className="text-platinum">Step 2:</strong> The server
                replies with its chosen configuration and sends its Digital
                Certificate (its cryptographic ID card) signed by a trusted
                Certificate Authority.
              </>
            )}
            {currentStep === 3 && (
              <>
                <strong className="text-platinum">Step 3:</strong> The client
                verifies the certificate. They then use Asymmetric Encryption
                (RSA/ECC) to securely exchange a symmetric &apos;session
                key&apos;.
              </>
            )}
            {currentStep === 4 && (
              <>
                <strong className="text-emerald-400">Secure!</strong> The
                handshake is complete. All further HTTP traffic (Request &
                Response) is now encrypted symmetrically (e.g., AES) using the
                shared session key, guaranteeing secrecy and integrity.
              </>
            )}
          </p>
        </div>
